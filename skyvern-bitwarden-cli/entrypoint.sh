#!/bin/bash
# Vendored, unmodified from github.com/skyvern-ai/skyvern/blob/main/bitwarden-cli-server/entrypoint.sh
# (AGPL-3.0, skyvern-ai/skyvern). Vendored here because that path is only shipped as a
# docker-compose build context upstream, not as a published ghcr.io image (see Dockerfile
# comment + task-4-report.md for the docker manifest inspect verification).
set -euo pipefail
